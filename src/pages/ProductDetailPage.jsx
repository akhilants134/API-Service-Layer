...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    getProductById(id)
      ...
      .then((res) => {
        setRelated(res.data.filter((p) => p.id !== parseInt(id)).slice(0, 3));
        setLoading(false);
      })
      .catch((err) => {
  ...
  // Use API service for add to cart
  ...
  // Use API service for review submission
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 flex flex-col gap-5">
          <div className="flex flex-wrap gap-2">
            ...
            <span
              className={`text-xs px-2 py-0.5 rounded-full ${product.pricing === "free" ? "bg-green-50 text-green-700" : "bg-blue-50 text-blue-700"}`}
            >
        ...
        <div className="flex flex-col gap-4">
          <div className="border border-gray-200 rounded-xl p-5 flex flex-col gap-3">
            <p
              className={`text-2xl font-bold ${product.pricing === "free" ? "text-green-600" : "text-gray-900"}`}
            >
              {product.pricing === "free" ? "Free" : `$${product.price}`}
            </p>
            <button
